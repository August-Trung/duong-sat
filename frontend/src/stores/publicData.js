...
function pickBetterPosition(currentBest, candidate) {
	...
	const currentAccuracy =
		currentBest.accuracy > 0
			? currentBest.accuracy
			: Number.POSITIVE_INFINITY;
	const candidateAccuracy =
		candidate.accuracy > 0 ? candidate.accuracy : Number.POSITIVE_INFINITY;
...
function watchBestPosition({
	...
	return new Promise((resolve, reject) => {
		...
		const handleSuccess = (position) => {
			...
			if (
				bestSample.accuracy > 0 &&
				bestSample.accuracy <= desiredAccuracy
			) {
...
async function readBestAvailablePosition() {
	...
	const collect = (position) => {
		bestSample = pickBetterPosition(
			bestSample,
			normalizePositionSample(position),
		);
	};
...
export async function loadPublicOverview(filters = {}) {
	...
	try {
		const [summary, layers, crossings, schedules, incidents] =
			await Promise.all([
				fetchSummary(),
				fetchLayers(),
...
export async function locateUser() {
	...
	try {
		...
		if (accuracy > MAX_LOCATION_ACCURACY_METERS) {
			throw new Error(
				"Vị trí hiện tại chưa đủ chính xác. Hãy bật GPS chính xác cao hoặc chọn vị trí trên bản đồ.",
			);
		}
